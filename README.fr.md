...
- Utilise un listage récursif par préfixe avec profondeur configurable pour une performance optimale sur les structures hiérarchiques
...
- `MAX_CONCURRENT_BUCKETS`: Limite le traitement concurrent des buckets (optionnel, aucune limite par défaut)
- `MAX_RECURSION_DEPTH`: Contrôle la profondeur de listage récursif par préfixe (défaut: 4)
  - Des valeurs plus élevées améliorent l'efficacité pour les structures de dossiers profondément imbriquées
  - Des valeurs plus faibles réduisent l'utilisation mémoire et préviennent le débordement de pile pour les structures extrêmement profondes
  - À la profondeur maximale, bascule vers un listage à plat sans préfixes
- `S3_PREFIX_DELIMITER`: Caractère utilisé comme séparateur de répertoire dans les clés d'objets S3 (défaut: "/")
  - Alternatives courantes: "-", "_", "|" pour différentes conventions de nommage
